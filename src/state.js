...
export const DEFAULT_SETTINGS = {
  ...
  recurEnabled:false,                 // 고급: 정기함(반복 일정 생성기) 사용 여부 — 기본 꺼짐(옵트인)
};
...
export const S = {
  ...
  /* 정기함(v2.3, v2.42 부활) — 반복 일정 정의들(보드 밖 생성기). reconcileRecur가
     도래 시 보드에 스폰. 고급 탭의 '정기함 사용'(settings.recurEnabled)이 켜져 있을
     때만 동작하며, 꺼져 있어도 정의 데이터는 로드/백업 왕복으로 보존된다. */
  recurDefs: [],
...
/* =========================================================================
   정기함 (v2.3, v2.42 부활) — 반복은 보드 밖 '정의(recurDef)'로 두고, 도래 시점에
   일반 메모를 보드에 스폰하는 생성기. recurDef = {id, memo, freq, dow?, time:{hh,mm},
...
/* 정기 정의들을 훑어 '도래한 회차'를 보드에 일반 메모로 스폰한다. 순수 변경(S만
   ...
   고급 탭의 '정기함 사용'(settings.recurEnabled)이 꺼져 있으면 아무것도 하지 않는다.
...
export function reconcileRecur(now=new Date()){
  if(!(S.settings && S.settings.recurEnabled)) return false;   // 옵트인 게이트(기본 꺼짐)
