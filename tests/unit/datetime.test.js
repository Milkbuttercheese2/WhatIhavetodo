...
test('fmtDue: 버킷 분류 (지남 강조 없음 · 임박 2시간)', () => {
  assert.equal(fmtDue(iso(-5)).cls, '');                     // 마감 지남 강조 제거
  assert.ok(!fmtDue(iso(-5)).label.endsWith('지남'));
  assert.equal(fmtDue(iso(30)).cls, 'soon');
  assert.ok(fmtDue(iso(30)).label.includes('분후'));
  assert.equal(fmtDue(iso(90)).cls, 'soon');                 // 2시간 이내 → 임박
  assert.ok(fmtDue(iso(90)).label.includes('시간후'));
  assert.equal(fmtDue(iso(180)).cls, '');                    // 2시간 초과 → 무클래스
});
