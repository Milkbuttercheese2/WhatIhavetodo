...
test('fmtDue: 긴급도 5단계 (지남=임박 동일 · 오늘/내일/이틀 · 그 이후 중립)', () => {
  ...
  assert.equal(fmtDue(dayAt(5,12)).cls, '');                  // 그 이후 → 중립
  // 오늘 늦은 시각: 2시간 초과면 u-today, 지금이 늦어 2시간 이내면 u-imm — 둘 다 허용
  assert.ok(['u-today','u-imm'].includes(fmtDue(dayAt(0,23)).cls));
});
