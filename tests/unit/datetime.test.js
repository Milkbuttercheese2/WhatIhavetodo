...
test('fmtDue: 버킷 분류', () => {
  assert.equal(fmtDue(iso(-5)).cls, 'late');
  assert.ok(fmtDue(iso(-5)).label.endsWith('지남'));
  assert.equal(fmtDue(iso(30)).cls, 'soon');
  assert.ok(fmtDue(iso(30)).label.includes('분후'));
  assert.equal(fmtDue(iso(180)).cls, 'soon');
  assert.ok(fmtDue(iso(180)).label.includes('시간후'));
  assert.equal(fmtDue(iso(600)).cls, '');                    // 4시간 초과 → 무클래스
});
