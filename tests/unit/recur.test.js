/* 정기함 도메인 — nextRecurDate · dayStart · reconcileRecur 생성기 (v2.42 부활).
   reconcileRecur는 settings.recurEnabled 게이트를 통과해야 동작한다(옵트인). */
...
/* 옵트인 게이트를 켠 상태로 리셋 — 대부분의 스폰 테스트가 켜짐을 전제로 한다 */
function reset(){ S.items=[]; S.recurDefs=[]; S.lastId=0; S.settings={recurEnabled:true}; }
...
test('reconcileRecur: recurEnabled 꺼짐이면 아무것도 안 하고 false (옵트인 게이트)', () => {
  reset(); S.settings.recurEnabled=false;
  const now=new Date(2026,6,10,10,0);
  S.recurDefs.push({id:1, memo:'야근', freq:'daily', dow:[], time:{hh:18,mm:0}, next:isoLocal(2026,6,10,18,0), paused:false});
  assert.equal(reconcileRecur(now), false);
  assert.equal(S.items.length, 0);   // 꺼짐이면 스폰 없음
});
