...
![version](https://img.shields.io/badge/version-v2.5.20-4d938a?style=for-the-badge)
...
- 이후에도 헤더의 **[저장 위치]** 버튼으로 언제든 옮길 수 있습니다. 실제 이동은 다음 재시작 시 그 시점의 최신 데이터로 수행됩니다. 이미 데이터가 들어 있는 폴더로는 기존 데이터를 덮어쓰지 않도록 이동이 거부됩니다. (v2.5.20)
- 저장은 항목을 수정하는 즉시 자동으로 이뤄집니다 (전체 트랜잭션, 부분 저장 없음).
- **저장이 실패하면(디스크 꽉 참·저장 위치 분리·백신 잠금 등) 화면 상단에 빨간 경고 배너가 뜹니다** — 조용히 유실되지 않도록. 다음 저장이 성공하면 자동으로 사라집니다. (v2.5.19)
- 자동 백업은 30분 간격 로테이션에 더해 일자별 첫 백업을 14일간 보존합니다. 항목이 한꺼번에 크게 줄어드는 저장(대량 삭제·통째 절단) 직전에는 스로틀과 무관하게 **덮어쓰기 전 스냅샷**을 따로 남깁니다. (v2.5.20)
...
- 시작 시 DB가 손상돼 있으면 최근 자동 백업으로 자동 복구합니다(백업도 무결성 검사를 통과한 것만 채택 — 손상된 최신 백업은 건너뛰고 더 오래된 정상 백업 사용, v2.5.20). 복구 불가 시에도 손상 파일을 보존한 채 새로 시작하며, 어떤 경우에도 앱이 실행 불능이 되지 않습니다.
...
| [`기능 제안서.md`](./기능%20제안서.md) | 공공기관 내부망 관점의 기능 추가 vs 단순 유지 제안 (보고서로만 유지, 개발 미착수) |
