...
- `db/mod.rs` — `open()` (pragmas + migrate), `integrity_check()`, `rotate_backup()` (timestamped `.sqlite` copies under a `backups/` dir, pruned to the newest N), `now_stamp()`. Backup files carry mixed prefixes (`wmhh_`, `wmhh_prerestore_`, `wmhh_preimport_`), so recovery and pruning MUST sort by the embedded `YYYYMMDD_HHMMSS` stamp via `stamp_key()`, never by raw filename — a plain lexical sort puts `'p' > '2'` and so picked a stale pre* snapshot as "newest", which restored old data and pruned the real latest backup (data-loss bug fixed in v2.5.11; regression tests cover it).
...
- `settings-menu.js` — header [설정] dropdown that hosts 저장 위치/백업/불러오기/XLSX/프리셋·식별정보 관리/주기 업무 입력·관리 buttons (ids unchanged — action listeners stay in their own modules). (The two Everything toggles were removed in v2.4.0.)
