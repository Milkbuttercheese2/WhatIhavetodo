...
- `commands.rs` — thin `#[tauri::command]` wrappers (`load_all`, `save_all`, `save_fields`, `save_presets`, `save_id_kinds`, `save_settings`, `save_recur_defs`, `backup_export`, `backup_import`) that just call into `db::*` and map errors to `String`. Business logic does **not** belong here or in `db/` — it stays in the frontend JS (see Migration-in-progress note above); Rust is CRUD-only.
...
- `state.js` — `S` (items/fields/presets/idKinds/settings/recurDefs/loaded/lastId/imported), `CORE_FIELDS` + `DEFAULT_*`, `newId()` (F12), `makeItem()` (single source of the item shape — new item fields get their default here), `toggleDone()` (domain op behind the card checkbox), `migrateItem()`, `reconcileCore()`, and the 정기함 generator (`nextRecurDate`/`dayStart`/`reconcileRecur` — spawns a normal item per due recurrence definition; see Data model). `S.imported` is the async handoff channel filled by `STORE.load()`/backup import and consumed by `reconcileImported()`.
...
- `recur-box.js` — 정기함 modal (v2.3): list/add/edit/pause/delete recurrence definitions (`S.recurDefs`), persisted via `STORE.saveRecurDefs`; spawning itself lives in `state.js::reconcileRecur`, hooked from `main.js` on load + the 60s tick.
- `capture-win.js` / `capture-boot.js` / `capture-bridge.js` (v2.23 global-shortcut mini capture) — `capture-win.js` runs ONLY in the capture webview and must not import main-app modules (store.js's top-level `__TAURI__` destructure would break, and module state would run twice); it emits the memo text as an event instead of saving. `capture-boot.js` is the capture.html-only entry so `capture-win.js` stays importable in tests. `capture-bridge.js` is the main-window side: receives the event and feeds `captureMemo()` so the F1 load gate/save queue/pending-merge all apply; access `__TAURI__.event` lazily inside functions, never at module top level.
