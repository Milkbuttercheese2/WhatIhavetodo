...
- `state.js` — `S` (items/fields/presets/idKinds/settings/loaded/lastId/imported), `CORE_FIELDS` + `DEFAULT_*`, `newId()` (F12), `makeItem()` (single source of the item shape — new item fields get their default here), `toggleDone()` (domain op behind the card checkbox; the hook point for future recurrence), `migrateItem()`, `reconcileCore()`. `S.imported` is the async handoff channel filled by `STORE.load()`/backup import and consumed by `reconcileImported()`.
