...
- `commands.rs` — thin `#[tauri::command]` wrappers (`load_all`, `save_all`, `save_fields`, `save_presets`, `save_id_kinds`, `save_settings`, `backup_export`, `backup_import`, …) that just call into `db::*` and map errors to `String`, plus the global mini-capture shortcut and settings-window setters: `CAPTURE_SHORTCUT` is the **default** (Ctrl+Alt+Space); `set_capture_shortcut` (live re-binding with unregister-old/register-new/rollback, backed by the `CaptureShortcut(Mutex<String>)` managed state in `lib.rs`) and `set_autostart` (HKCU Run toggle) were **removed in v2.31 and restored in v2.4** for the settings window. `lib.rs` reads the actual shortcut from `settings.captureShortcut` at startup, falling back to the const. (`save_recur_defs` stays removed — 정기함 is gone.) Business logic does **not** belong here or in `db/` — it stays in the frontend JS (see Migration-in-progress note above); Rust is CRUD-only.
...
- `settings.js` (v2.4 설정 창) — the ⚙ header button opens a left-tab (일반/알림/단축키/데이터) settings modal reusing the `.modal-bg`+`modal-split` pattern. Exports `initSettings()` (wiring), `applyTheme()` (sets `<html data-theme>`; called by `reconcileImported` so the stored theme survives initial load + JSON restore), and pure `resolveTheme(pref)`. Handlers mutate `S.settings.*` then persist via `STORE.saveSettings`; `autostart`/`captureShortcut` also `invoke()` the Rust setters and roll back the control on Err. Dark theme is a `:root[data-theme="dark"]` override block in `styles.css` (+`color-scheme:dark`). The 데이터 tab hosts the moved 저장 위치/백업/복원/XLSX buttons (same IDs — `initBackup()` binds by id).
- `capture-win.js` / `capture-boot.js` / `capture-bridge.js` (v2.23 global-shortcut mini capture; default shortcut Ctrl+Alt+Space, re-bindable via the v2.4 settings window) — `capture-win.js` runs ONLY in the capture webview and must not import main-app modules (this includes `settings.js`) (store.js's top-level `__TAURI__` destructure would break, and module state would run twice); it emits the memo text as an event instead of saving. `capture-boot.js` is the capture.html-only entry so `capture-win.js` stays importable in tests. `capture-bridge.js` is the main-window side: receives the event and feeds `captureMemo()` so the F1 load gate/save queue/pending-merge all apply, plus the one-time hidden-to-tray notice toast; access `__TAURI__.event` lazily inside functions, never at module top level.

Removed in v2.31 (kept here so old references make sense): `recur-box.js` (정기함 modal), the `state.js` recurrence generator (`reconcileRecur` etc.), the `saveStatus`/`setStatus` save-failure indicator, and the ⚡ 빠른 메모 settings modal (shortcut recorder + tray/autostart toggles). **v2.4 note:** the settings surface returns as `settings.js` (⚙ 설정 창) — the shortcut recorder and the closeToTray/autostart/autostartMinimized toggles are restored there (plus new theme + alarm-sound options), and the `set_capture_shortcut`/`set_autostart` Rust commands are back. Only 정기함 (`recur-box.js`, `reconcileRecur`) and the `saveStatus` indicator stay removed.
